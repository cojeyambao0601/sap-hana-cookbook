...
if !File.exists?("#{node['hana']['installpath']}/#{node['hana']['sid']}/HDB#{node['hana']['instance']}/#{node[:hostname]}/install.finished")
  ...
  if node['hana']['checkhardware'] && "#{node['hana']['checkhardware']}".chomp == "true"
    hana_install_worker_command = "export HDB_COMPILEBRANCH=1 && export HDB_IGNORE_HANA_PLATFORM_CHECK=1 && #{hana_install_worker_command} --ignore=check_diskspace,check_min_mem"
  end
